...
test("los cultos guardados en otro equipo se refrescan sin pisar trabajo local", () => {
  ...
  assert.match(control, /resolverCambioCultoRemoto/)
  assert.match(control, /hayCambiosLocales: hayCambiosCulto/)
  assert.match(control, /enProyeccion: indiceLista !== null \|\| !!activaId/)
  ...
  assert.match(control, /hayConflictoCultoPersistido\(firmaItemsBDRef\.current, firmaActualBD\)/)
  assert.match(control, /Este culto fue modificado desde otro equipo después de que lo abriste/)
  assert.match(control, /Guardar como copia/)
})
...
test("Transmisión sincroniza diseño sin subir cámaras, micrófonos ni claves", () => {
  ...
  assert.match(enVivo, /guardarConfiguracionNube\(iglesiaId, "transmision", configuracion\)/)
  const inicio = enVivo.indexOf("const configuracion: ConfiguracionTransmisionNube")
  ...
  assert.match(enVivo, /window\.addEventListener\("focus", revisar\)/)
  assert.match(enVivo, /window\.setInterval\(revisar, 30000\)/)
  assert.match(enVivo, /firma === ultimaConfigNubeRef\.current/)
...
test("Control comparte apariencia segura y evita publicar rutas locales", () => {
  const control = fs.readFileSync("app/control/page.tsx", "utf8")
  assert.match(control, /guardarConfiguracionNube\(iglesiaIdActual, "control", configuracion\)/)
  ...
  assert.match(control, /window\.addEventListener\("focus", revisar\)/)
  assert.match(control, /window\.setInterval\(revisar, 30000\)/)
  assert.match(control, /firma === ultimaConfigControlNubeRef\.current/)
})
